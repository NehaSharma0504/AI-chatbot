...
1. Install JDK 17 or newer (Temurin / Oracle / OpenJDK)
...
```bash
javac VectorDbServer.java
```

This compiles the backend server class.

> **Troubleshooting:**
> - `javac: command not found` → JDK not installed / PATH not set
> - `class file has wrong version` → old Java runtime; upgrade to JDK/JRE 17+
...
```
VectorDB/
├── VectorDbServer.java  ← Java backend (HNSW, KD-Tree, BruteForce, REST API, RAG)
├── index.html           ← Frontend (PCA scatter plot, chat UI, benchmark)
├── main.cpp             ← Legacy C++ implementation (reference)
├── httplib.h            ← Legacy C++ dependency
└── README.md            ← This file
```

### Architecture (VectorDbServer.java)
...
| `javac: command not found` | Install JDK 17+ and add it to PATH |
...
Then edit [VectorDbServer.java](VectorDbServer.java) where `genModel` is set:
```java
